...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  })}</div>  <div class="reviewAndRating"><div class="flex min-h-[140px] w-full items-center py-10 justify-center rounded-lg lg:overflow-visible"><div class="flex flex-col items-center gap-6 font-bold text-blue-gray-500"><h1 class="text-2xl pb-2" data-svelte-h="svelte-1gvkcob">You have already visited this place. Why not rate it now?</h1> <div class="inline-flex items-center"><form method="post" action="?/myform"${add_attribute("this", ratingForm, 0)}><input type="hidden" name="spotId"${add_attribute("value", data.id, 0)}> <div class="rating flex justify-center flex-row-reverse gap-5 group">${each(Array(5), (_, i) => {
    return `<input class="hidden peer" type="radio"${add_attribute("id", `star${5 - i}`, 0)}${add_attribute("value", 5 - i, 0)} name="selRating"${5 - i === $selectedRating ? add_attribute("checked", true, 1) : ""}> <label class="text-gray-500 text-3xl cursor-pointer peer-checked:text-orange-400 peer-hover:text-orange-400 group-hover:peer-checked:text-orange-400"${add_attribute("for", `star${5 - i}`, 0)}>★
              </label>`;
  })}</div> <div class="mt-4"><p>Selected Rating: ${escape($selectedRating)}</p></div></form></div></div></div></div>`;
});
export {
  Page as default
};
